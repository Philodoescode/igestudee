...
export default function AdminProfilePage() {
  ...
  useEffect(() => {
    async function fetchProfile() {
      ...
      if (error) { toast.error("Failed to load profile data."); console.error(error); } 
        ...
        const fetchedData = data as unknown as AdminProfileData;
        setProfileData(fetchedData);
        setFormData({ first_name: fetchedData.first_name || '', last_name: fetchedData.last_name || '', email: fetchedData.email || '' });
      }
      setIsLoading(false);
    }
    fetchProfile();
  ...
  const handleCancel = () => {
    if (profileData) { setFormData({ first_name: profileData.first_name || '', last_name: profileData.last_name || '', email: profileData.email || '' }); }
    setIsEditing(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <p className="text-gray-600 mt-1">Manage your administrator account details.</p>
      </motion.div>
      <Card>
            ...
            <TabsContent value="personal" className="mt-6">
              <form onSubmit={handleSave}>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div className="space-y-1.5"><Label htmlFor="email">Email Address</Label><Input id="email" type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} disabled={!isEditing} /></div></div>
                </div>
              </form>
            </TabsContent>

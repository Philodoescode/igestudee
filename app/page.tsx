...
import { taDashboardData } from "@/lib/database"
...
export default function TADashboard() {
  ...
  const totalStudents = taDashboardData.studentGroups.reduce((sum, group) => sum + group.studentCount, 0)
  const totalPendingQuestions = taDashboardData.studentGroups.reduce((sum, group) => sum + group.pendingQuestions, 0)
  const todaySessions = taDashboardData.upcomingSessions.filter((session) => {
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold">{taDashboardData.studentGroups.length}</p>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <Card className="h-fit">
            ...
            <CardContent className="space-y-4">
              {taDashboardData.studentGroups.map((group) => (
                <div
                  key={group.id}
        ...
        >
          <Card className="h-fit">
            ...
            <CardContent className="space-y-4">
              {taDashboardData.forumActivity.map((question) => (
                <div
                  key={question.id}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <Card>
            ...
            <CardContent className="space-y-4">
              {taDashboardData.upcomingSessions.slice(0, 3).map((session) => (
                <div
                  key={session.id}
                  className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors"
                >
        ...
        >
          <Card>
            ...
            <CardContent className="space-y-4">
              {taDashboardData.announcements.map((announcement) => (
